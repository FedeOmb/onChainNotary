...
import { Box, Flex, Heading, Button, Text, SegmentGroup, VStack, FileUpload, Image, Alert, DataList } from "@chakra-ui/react";
...
export default function Upload() {
  ...
  //verifica se il documento è in un formato supportato
  ...
  //calcola l'hash per il documento caricato
  ...
  //carica l'immagine selezionata e crea l'anteprima
  ...
  const handleImageReject = (e) => {
    ...
    setUploadError("Formato o dimensione dell'immagine non supportati.");
  }
  const handleFileReject = (e) => {
    console.error("File non accettato:", e);
    setUploadError("Formato o dimensione del documento non supportati.");
  }

  //calcola gli hash per l'immagine caricata
  ...
  //resetta le variabili di stato del componente
  function clearState() {
    setFile(null);
    setImage(null);
    setDocHashData(null);
      ...
      <Flex justify="center" width="90vw" p={4}>
        ...
        {fileType === "document" && (
          <Flex flexDirection="row" flexWrap="wrap" flexGrow="1" p={4} justifyContent={"center"} gap="2">
            <FileUpload.Root maxW="xl" flexBasis="50%" flexGrow="1" alignItems="stretch"
              maxFiles={1} maxFileSize="50000000" onFileAccept={handleFileChange} onFileReject={handleFileReject}>
                ...
                <FileUpload.DropzoneContent>
                  {file ? (
                    ...
                    <>
                      ...
                      <Box color="fg.muted">sono supportati i più comuni formati di documenti e testo come PDF, Microsoft Office, OpenOffice e altri fino a 50 MB</Box>
                      <Box>
                        {uploadError && (
        ...
        {fileType === "image" && (
          <Flex flexDirection="row" flexWrap="wrap" flexGrow="1" p={4} justifyContent={"center"} gap={2}>
            <FileUpload.Root maxW="xl" flexBasis="50%" flexGrow="1" alignItems="stretch"
              maxFiles={1} accept={acceptedImageTypes.join(",")} maxFileSize="20000000" onFileAccept={handleImageChange} onFileReject={handleImageReject}>
              <FileUpload.HiddenInput />
              <FileUpload.Dropzone>
                <FileUpload.DropzoneContent>
                  {image ? (
