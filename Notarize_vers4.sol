...
contract NotarizationDapp {
    ...
    ) external 
    {
        ...
        bytes32 docKey = keccak256(abi.encode(_hashAlgorithm, _docHash));        
        require(
    ...
    ) external 
    {
        ...
        bytes32 imgKey = keccak256(abi.encode(_imageHashAlgorithm,_imageHash));
    ...
    /// @param _hashAlgorithm: algorithm used to generate document hash
    ...
    /// @param _imageHashAlgorithm: algorithm used to generate document hash
    ...
    function getMyDocumentsKeys()
        external view returns (bytes32[] memory docKeys)
    {
    ...
    function getMyImagesKeys()
        external view returns (bytes32[] memory imageKeys)
    {
    ...
    function getMyFilesCount()
        external view returns (uint256 docsCount, uint256 imgCount)
    {
        return (
    ...
    function imageExistsByHash(
        ...
        bytes32 imgKey = keccak256(abi.encode(_imageHashAlgorithm, _imageHash));
        return keyToImage[imgKey].uploader != address(0);
    }
}
